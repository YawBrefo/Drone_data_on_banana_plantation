...
`A05_Banana_census.ipynb` converts ML detection output (CSVs + GeoTIFF tiles) into a geolocated, boundary-clipped GeoJSON of banana plants, with each point classified as either a **Parent** stalk or a **Sucker** offshoot using DSM-derived surface height. This is for a plantation block ideal for exteding to large field sizes.
