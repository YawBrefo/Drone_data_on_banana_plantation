...
`A05_Banana_census.ipynb` converts ML detection outputs (CSVs + GeoTIFF tiles) into a geolocated, boundary-clipped GeoJSON of banana plants, with each vector point classified as either a **Parent** stalk or a **Sucker** offshoot using DSM-derived surface height. This is for a plantation block ideal for extending to large field sizes.
